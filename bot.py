...
# 100 limit thread pool for ultra-fast OTP processing
...
DEFAULT_CONFIG = {
    "bot_token": "8695687684:AAFcRDobntm55fqAw6FMBMIzL09ec2QKeR0",
    ...
    "group_bot": {
        "enabled": True,
        "token": "8695687684:AAFcRDobntm55fqAw6FMBMIzL09ec2QKeR0",
...
def process_and_send_email(raw_email, base_email):
    try:
        ...
        elif ("otp-noreply" in from_.lower() or "imo verification code" in subject.lower()) and otp and tag:
            user = find_user_by_tag(tag)
            if user:
                send_otp_to_user(user, otp, tag=tag, service="IMO OTP")

        # WhatsApp OTP → user কে পাঠাও
        elif ("whatsapp.com" in from_.lower() or "whatsapp" in subject.lower()) and otp and tag:
            user = find_user_by_tag(tag)
            if user:
        ...
        print(f"{RED}[Async Process Error] {e}{RESET}")
...
def process_unseen(mail_conn, base_email):
    try:
        ...
        for num in msgs[0].split():
            ...
            # Submit the raw mail processing Task to ThreadPoolExecutor to run concurrently
            raw_email = data[0][1]
            executor.submit(process_and_send_email, raw_email, base_email)

            # এই mail process করা শুরু হয়েছে — uid update করো এবং Seen মার্ক করো
...
def imap_thread(base_email, app_password):
    """Robust IMAP thread with IDLE push + fallback polling every 5s."""
    IDLE_TIMEOUT = 25  # seconds to wait for IDLE response before re-issuing
    POLL_INTERVAL = 5  # seconds between polls in fallback mode (was 0.5s — too aggressive)
    reconnect_delay = 5  # start with 5s, grow on repeated failures

    # Build a permissive SSL context to avoid _ssl.c EOF issues
    ...
    while True:
        ...
        try:
            ...
            if use_idle:
                print(f"{GREEN}[IMAP IDLE] {base_email}{RESET}")
                import socket as _socket
                while True:
                    try:
                        # Send IDLE command
                        tag_bytes = mail._new_tag()
                        mail.send(tag_bytes + b' IDLE\r\n')
                        # Wait for "+ idling" continuation
                        mail.readline()

                        # Wait up to IDLE_TIMEOUT seconds for EXISTS or server data
                        ...
                        # Reset timeout and process
                        mail.socket().settimeout(None)
                        mail.select("inbox")
                        process_unseen(mail, base_email)

                    except (OSError, EOFError, imaplib.IMAP4.abort) as idle_e:
                        print(f"{YELLOW}[IDLE loop] {idle_e}{RESET}")
                        break  # reconnect
                ...
                print(f"{YELLOW}[IMAP POLL] {base_email}{RESET}")
                noop_counter = 0
                while True:
                    ...
                    try:
                        mail.select("inbox")
                        process_unseen(mail, base_email)
                    except (OSError, EOFError, imaplib.IMAP4.abort) as poll_e:
                        print(f"{YELLOW}[IMAP POLL error] {base_email}: {poll_e}{RESET}")
                        break  # reconnect
                    ...
                    if noop_counter >= 60:
                        try:
                            ...
                            break  # reconnect if noop fails
                        noop_counter = 0

        except (OSError, EOFError, imaplib.IMAP4.abort) as e:
            print(f"{RED}[IMAP reconnect] {base_email}: {e}{RESET}")
        except Exception as e:
            print(f"{RED}[IMAP reconnect] {base_email}: {e}{RESET}")
        finally:
            if mail:
        ...
        print(f"{YELLOW}[IMAP] Reconnecting {base_email} in {reconnect_delay}s...{RESET}")
